...
class BCDiceMaker
  ...
  def newSMDice
    SMDice.new(self, @cardTrader, @diceBot, @counterInfos, @tableFileData)
  end
...
class SMDice
  ...
  def isMaster
    (@nick_e == @parent.master) or (@parent.master == '')
  end
  ...
  def setPrintPlotChannelIfChannelUndefine
    ...
    channel = getPrintPlotChannel(@nick_e)
    setPrintPlotChannel if channel
  end
  ...
  def getTableDataResult(arg)
    ...
    when /(\d+)D(\d+)/i
      dice_count = $1
      side_num = $2
      value, = roll(dice_count, side_num)
    else
      table = []
    end
  ...
  def roll(dice_cnt, dice_max, dice_sort = 0, dice_add = 0 , dice_ul = '' , dice_diff = 0 , dice_re = nil)
    ...
    unless (dice_cnt <= $DICE_MAXCNT) and (dice_max <= $DICE_MAXNUM)
      return total, dice_str, number_spot1, cnt_max, n_max, cnt_suc, cnt_re
    end
  ...
  def randFromRands(target_max)
    next_rand = @rands.shift
    ...
    if max != target_max
      #return randNomal(targetMax)
      raise "invalid max value! value, max, targetMax: #{value}, #{max}, #{target_max}"
    end
  ...
  def bdice(string) # 個数判定型ダイスロール
    ...
    string = $2
    if $5
      inequality = marshalSignOfInequality( $6 )
      diff = $7.to_i
      string = $3
    elsif /([<>=]+)(\d+)/ =~ @diceBot.defaultSuccessTarget
      inequality = marshalSignOfInequality($1)
      diff = $2.to_i
    end
    ...
    dice_a.each do |dice_o|
      ...
      dice_dat = roll(dice_cnt, dice_max, (@diceBot.sortType & 2), 0, inequality, diff)
      suc += dice_dat[5]
      output += ',' if(output != '')
      output += dice_dat[1]
      number_spot1 += dice_dat[2]
      dice_cnt_total += dice_cnt
    end
    
    if inequality != ''
      string += "#{inequality}#{diff}"
      output = "#{output} ＞ 成功数#{suc}"
      output += @diceBot.getGrichText(number_spot1, dice_cnt_total, suc)
    end

    "#{@nick_e}: (#{string}) ＞ #{output}"
  end
  ...
  def openSecretRoll(channel, mode)
    ...
    member_key = getSecretRollMembersHolderKey(channel, mode)
    members = $secretRollMembersHolder[member_key]
    ...
    members.each do |member|
      dice_result_key = getSecretDiceResultHolderKey(channel, mode, member)
      debug('openSecretRoll diceResulyKey', dice_result_key)
      
      dice_result = $secretDiceResultHolder[dice_result_key]
      debug('openSecretRoll diceResult', dice_result)
      
      if dice_result
        messages.push( dice_result )
        $secretDiceResultHolder.delete(dice_result_key)
      end
    end
  ...
  def getNick(nick = nil)
    ...
    $1   # Nick端の数字はカウンター変わりに使われることが多いので除去
  end
  ...
  def saveSecretDiceResult(dice_result, channel, mode)
    nick = getNick()
    
    if mode != 0
      dice_result = "#{nick}: #{dice_result}" # プロットにNickを追加
    end
    
    key = getSecretDiceResultHolderKey(channel, mode, nick)
    $secretDiceResultHolder[key] = dice_result;    # 複数チャンネルも一応想定
  ...
  def getMarshaledSignOfInequality(text)
    return '' unless text
    marshalSignOfInequality(text)
  end
  
  def marshalSignOfInequality(inequality)  # 不等号の整列
    case inequality
    when /(<=|=<)/
      return '<='
    when /(>=|=>)/
  ...
  def check_hit(dice_now, inequality, diff) # 成功数判定用
    suc = 0
  ...
  def check_suc(*check_param)
    ...
    check_param_new = [total_n, dice_n, signOfInequality, diff, dice_cnt, dice_max, n1, n_max]
    
    text = getSuccessText(*check_param_new)
  ...
  def parren_killer(string)
    ...
    while /^(.*?)\[(\d+[Dd]\d+)\](.*)/ =~ string
      ...
      str_before = $1 if($1)
      str_after = $3 if($3)
      rolled, _ = rollDiceAddingUp(dice_cmd)
  ...
  def changeRangeTextToNumberText(text)
    debug('[st...ed] before string', text)
    ...
    while /^(.*?)\[(\d+)[.]{3}(\d+)\](.*)/ =~ text
      ...
      if range_begin < range_end
        range = (range_end - range_begin + 1)
        debug('range', range)
        
        rolled_number, = roll(1, range)
        result_number = range_begin - 1 + rolled_number
        text = "#{before_text}#{result_number}#{after_text}"
      end
    end
    
    debug('[st...ed] after string', text)
  ...
  def setGameByTitle(game_title)  # 各種ゲームモードの設定
    debug('setGameByTitle gameTitle', game_title)
    ...
    when /(^|\s)None$/i, ''
      ...
      dicebot = loader.loadUnknownGame(game_title)
    end

    dicebot = DiceBot.new unless dicebot
